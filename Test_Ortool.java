...
public class Test_Ortool {
	...
	int[][] t = { { 0, 3, 6, 6, 4, 3, 5, 6 }, // thoi gian di chuyen
			{ 3, 0, 5, 4, 2, 6, 2, 1 }, { 6, 5, 0, 4, 9, 1, 9, 4 }, { 6, 4, 4, 0, 7, 2, 6, 4 },
			{ 4, 2, 9, 7, 0, 9, 4, 25 }, { 3, 6, 1, 2, 9, 0, 6, 4 }, { 5, 2, 9, 6, 4, 6, 0, 3 },
			{ 6, 1, 4, 4, 25, 4, 3, 0 } };
	...
	public void solve() {
		...
		for (int u = 1; u <= k; u++) {
			...
			int i = 1;
			while (x[0][i][u].solutionValue() == 0 && i <= n) i++;
			if (i > n) break;
			System.out.print("0 -> "+ i);
			int j = 1;
			while (true) {
				while (j <= n && x[i][j][u].solutionValue() == 0) j++;
				if (j > n) break;
				System.out.print(" -> " + j);
				i = j;
				j = 1;
	...
	public static void main(String[] args) {
		Test_Ortool app = new Test_Ortool();
		app.solve();
	}
}
